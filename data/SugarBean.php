...
class SugarBean
{
    ...
    public static function get_union_related_list(
        ...
        if (empty($final_query)) {
            ...
            foreach ($subqueries as $subquery) {
                ...
                if ($order_by &&
                        isset($subpanel_def->panel_definition['list_fields'][$order_by]['widget_class']) &&
    ...
    public function get_import_required_fields()
    {
        ...
        foreach ($importable_fields as $name => $properties) {
            if (isset($properties['importable']) &&
                    is_string($properties['importable']) &&
            ...
            foreach ($fieldDefs as $key => $value_array) {
                if ((isset($value_array['importable']) &&
                    ...
                    if (isset($value_array['type']) &&
                            ($value_array['type'] == 'relate') &&
                            isset($value_array['id_name'])) {
                        $idField = $value_array['id_name'];
                        if (isset($fieldDefs[$idField]) &&
                                isset($fieldDefs[$idField]['type']) &&
    ...
    public function save($check_notify = false)
    {
        ...
        if ($this->hasEmails() &&
                !empty($this->email_addresses_non_primary) &&
    ...
    public function cleanBean()
    {
        if (!is_array($this->field_defs) && !is_object($this->field_defs)) {
            ...
            foreach ((array) $this->field_defs as $key => $def) {
                ...
                // if (
                //     isset($def['type']) &&
                //     in_array($def['type'], ['name', 'varchar']) &&
                //     property_exists($this, $key) &&  // ✅ make sure the property exists
                //     !is_null($this->$key)
                // ) {
                //     $this->$key = trim($this->$key);
                // }
                ...
                if (
                    isset($def['type']) &&
                    in_array($def['type'], ['name', 'varchar']) &&
                    isset($this->$key) &&              // ✅ checks property is set & not undefined
                    !is_null($this->$key)
                ) {
                    $this->$key = trim($this->$key);
                }
    ...
    protected function set_relationship_info($exclude = array())
    {
        ...
        if ($rel_id && $rel_link && !in_array($rel_link, $exclude) && $rel_id != $this->id) {
            ...
            if ($isRelationshipLoaded &&
                !empty($this->$rel_link) &&
                $this->$rel_link->getRelationshipObject() &&
            ...
            foreach ($this->field_defs as $def) {
                if (!empty($def['type']) && $def['type'] == 'parent') {
                    ...
                    if (
                        ...
                        (
                            !empty($this->fetched_row[$typeField]) &&
                            !empty($this->fetched_row[$idField]) &&
                        ...
                        if (!empty($this->fetched_row[$typeField]) &&
                                !empty($this->fetched_row[$idField]) &&
    ...
    {
        if ($check_notify ||
            (isset($this->notify_inworkflow) && $this->notify_inworkflow) &&
    ...
    public function create_new_list_query(
        ...
        foreach ($fields as $field => $value) {
            if (isset($this->field_defs[$field]) &&
                    isset($this->field_defs[$field]['source']) &&
                ...
                if (!empty($this->$linkField)) {
                    ...
                    $table_joined = !empty($joined_tables[$params['join_table_alias']]) ||
                        (!empty($joined_tables[$params['join_table_link_alias']]) &&
                            ...
                            $data['link_type'] == 'relationship_info');
            ...
            if ($currency) {
                ...
                foreach ($this->field_defs as $this_field) {
                    if (isset($this_field['type']) &&
                            $this_field['type'] == 'relate' &&
                            isset($this_field['module']) &&
                            $this_field['module'] == 'Currencies' &&
                            isset($this_field['id_name']) &&
                            $this_field['id_name'] == 'currency_id') {
    ...
    public function fill_in_link_field($linkFieldName, $def)
    {
        ...
        if (!empty($this->field_defs[$linkFieldName]['type']) &&
                $this->field_defs[$linkFieldName]['type'] == 'id' &&
                !empty($def['link'])) {
        ...
        if ($is_owner === 'not_set') {
            ...
            if ($view == 'edit' && !$is_owner && !empty($this->id)) {
                ...
                if (!empty($this->fetched_row) &&
                        ...
                        !empty($this->fetched_row['created_by'])) {
